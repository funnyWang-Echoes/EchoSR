...
def buildDAT(upscale=4):
    return DAT(
        ...
        embed_dim=60,
        num_heads=[6],
        expansion_factor=2,  # small: 2 [8,16] normal:4 [8,32]
        resi_connection='3conv', #light 3conv [8,32]
        split_size=[8, 32],
    )
...
def main():
    ...
    model = buildDAT(4).to(device).eval()
...
def main():
    ...
    # 2. 创建模型
    model = buildDAT(2).to(device).eval()
