...
class CAB(nn.Module):
    ...
    def __init__(self, num_feat, compress_ratio=3, squeeze_factor=24):
...
def main():
    ...
    # 创建模型实例
    model = HAT().to(device).eval()

    # 随机生成输入数据
    input_shape = (1, 3, 640, 640)  # 假设批次大小为1
    ...
    print(f"Average inference time over {num_inferences} runs: {average_inference_time:.4f} seconds")
...
def simulate_training():
    ...
    # 随机生成输入数据和目标数据
    input_shape = (16, 3, 36, 36)  # 假设批次大小为1
    input_data = torch.randn(input_shape).to(device)
    target = torch.randn(model(input_data).shape).to(device)
    ...
    print(f"Input shape: {input_shape}")
    print(f"Output shape: {output.shape}")
    print(f"Average training time per epoch over {num_epochs} epochs: {average_epoch_time:.4f} seconds")
    print(f"Memory allocated: {memory_allocated:.2f} MB")
    print(f"Max memory allocated: {max_memory_allocated:.2f} MB")
    print(f"Max memory reserved: {max_memory_reserved:.2f} MB")
    print(f"Number of parameters: {params / 1e6:.2f} M")
