...
class CAB(nn.Module):
    def __init__(self, num_feat, is_light_sr= True, compress_ratio=3,squeeze_factor=30):
...
def main():
    ...
    model = buildMambaIR().to(device).eval()
...
def simulate_training():
    ...
    # 随机生成输入数据和目标数据
    input_shape = (16, 3, 48, 48)  # 假设批次大小为1
    input_data = torch.randn(input_shape).to(device)
...
def main():
    ...
    # 2. 创建模型
    model = buildMambaIR(2).to(device).eval()
