...
class SCC(nn.Module):
    ...
    def spatial_linear_projection(self, x):
        ...
        x = (x.view(B, num_h, map_H, H // map_H, map_W, W // map_W, C).permute(0, 1, 2, 4, 6, 3, 5).
             contiguous().view(B,num_h, map_H * map_W, C, -1))
...
def buildHit(upscale=4):
    return HiT_SIR(upscale=upscale,
                   base_win_size=[8, 8], img_range=1., depths=[6, 6, 6, 6],
...
def main():
    ...
    model = buildHit(2).to(device).eval()
