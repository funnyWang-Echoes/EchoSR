...
def save_activation_mean_maps(activations_list, imgnames, save_dir, prefix, original_images):
    ...
    branch_names = ['Image', 'Identity', r'$k = 5 \times 5$', r'$k = 11 \times 11$', r'$k = 17 \times 17$']
    ...
    # 减少图片间的间距 - 可以调整这些值来控制间距
    # wspace控制水平间距，hspace控制垂直间距（红线处）
    # 调整这里的值可以减小垂直间距，值越小间距越小
    plt.subplots_adjust(wspace=0.01, hspace=-0.15)  # 可以尝试负值进一步减小垂直间距
...
def register_hooks(model):
    ...
    def get_activation(name, prefix, is_egem=False):
        def hook(model, input, output):
            if not is_egem and 'ISBlock' in name:
                try:
    ...
    hooks = []
    target_layers = [
        ('layers.0.residual_group.blocks.0.EGEM', 'first_layer_first_block', True),  # Add EGEM for first block
        ('layers.0.residual_group.blocks.0.ISBlock', 'first_layer_first_block', False),
        ('layers.3.residual_group.blocks.4.EGEM', 'last_layer_last_block', True),  # Add EGEM for last block
        ('layers.3.residual_group.blocks.4.ISBlock', 'last_layer_last_block', False),
    ]

    for layer_name, prefix, is_egem in target_layers:
        try:
            layer = dict([*model.named_modules()])[layer_name]
            handle = layer.register_forward_hook(get_activation(layer_name, prefix, is_egem))
            hooks.append(handle)
...
def main():
    ...
    parser.add_argument('--input', type=str, default='Urban100/LR_bicubic/X4', help='input test image folder')
    parser.add_argument('--output', type=str, default='./ActMap/v3', help='output folder')
