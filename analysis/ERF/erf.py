...
from model_zoo.EchoSR import buildModel
import os
import time
...
root_path = 'F:\code\CodeProject\EchoSR\options\EchoSR_test\test_EchoSR_light_SRx4.yml'
...
def visualize_erf(MODEL: nn.Module = None, num_images=100, data_path="/mnt/data/WorkSpace/wbh/myMambaIR/datasets/SR/Urban100/LR_bicubic",
    ...
    def main(args, MODEL: nn.Module = None):
        ...
        for idx,data_sample in enumerate(test_loader):
            ...
            samples = F.interpolate(data_sample['lq'],size=(200,200))
...
if __name__ == '__main__':
    ...
    # init_model = buildMambaIR(2)
    init_model = buildModel(4)
    # ckpt_path = '/mnt/data/WorkSpace/wbh/myMambaIR/analysis/ckpt/mambaIR_lightSR2.pth' # path to load your pre_trained model weights
    ckpt_path = '/mnt/data/WorkSpace/wbh/myMambaIR/experiments/EchoSR_SR_x4_light_DF2K/models/net_g_510000.pth' # path to load your pre_trained model weights
    init_model.load_state_dict(torch.load(ckpt_path)['params'])
    save_path = f"./tmp/{time.time()}/erf.npy"
    grad_map = visualize_erf(init_model, save_path=save_path)
    analyze_erf(source=grad_map, dest=f"{showpath}/4x_erf.png")
